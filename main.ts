...
export default class GoogleDrive extends Plugin {
    ...
    parseSettings(settings: GoogleDriveSettings) {
        ...
        if (this.syncIntervalSecs != syncIntervalSecs) {
            ...
            if (this.syncIntervalSecs != 0 && this.settings.autoSynchronize) {
                this.syncIntervalId = window.setInterval(() => {
					logInfo("Attempting automatic synchronization");

					if (!this.settings.accessToken || !this.settings.folderId) {
						logWarn("Google Drive not logged in. Please login to enable this functionality");
						this.settings.autoSynchronize = false;
...
class GoogleDriveSettingsTab extends PluginSettingTab {
    ...
    display(): void {
		...
		// XXX_jwir3: Fix this so that there is a list of authors here.
        // containerEl.createEl("small", { text: "Created by "})
        //     .appendChild(createEl("ul")
		// 		.appendChild(createEl("li"))
		// 		.appendChild(createEl("li")));
		...
		const autoSyncSetting = new Setting(containerEl)
			.setName("Synchronize Automatically")
			.setTooltip("Whether to synchronize automatically based on a timer")
			.setDesc("Enable this and set a value for the synchronization interval to automatically sync to Google Drive.")
			.addToggle((automaticSyncToggle) => {
				automaticSyncToggle
					.setValue(this.plugin.settings.autoSynchronize)
					.onChange(async (val) => {
						logInfo("Automatic synchronization set to: " + val);
						this.plugin.settings.autoSynchronize = val;
						await this.plugin.saveSettings();
						this.display();
					});
			})

			if (this.plugin.settings.autoSynchronize) {
				autoSyncSetting
				.addText(text => text
					.setPlaceholder("3600")
